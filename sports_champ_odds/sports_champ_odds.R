...
get_odds_data <- function(url){
  ...
  return(data.frame(html_table(tableguy, fill=TRUE)[1]))
}
...
scrape_n_plot <- function(website) {
  df <- get_odds_data(website)
...
nba_plot <- scrape_n_plot(nba_site)
mlb_plot <- scrape_n_plot(mlb_site)
nhl_plot <- scrape_n_plot(nhl_site)
nfl_plot <- scrape_n_plot(nfl_site)
...
# MLB is now grabbing odds for world championship, not world series
...
nhl_data <- get_odds_data(nhl_site)
nhl_data$sport <- "nhl"

nfl_data <- get_odds_data(nfl_site)
nfl_data$sport <- "nfl"

mlb_data <- get_odds_data(mlb_site)
mlb_data$sport <- "mlb"
...
write.csv(all_sports, "sports_odds_2016-01-02.csv", row.names=FALSE)
...
# Next steps:
#  - scrape win/loss records over years to create line charts by sport
...
nba_records <- "http://www.basketball-reference.com/leagues/NBA_2017_standings.html"
webpage <- read_html(nba_records)
tableguy <- html_nodes(webpage, xpath='//*[@class="sortable stats_table now_sortable"]')
#tableguy <- html_nodes(webpage, xpath='table[@class="sortable stats_table now_sortable"]')
#tableguy <- html_nodes(webpage, xpath='//*[@id="expanded_standings"]')
#tableguy <- html_nodes(webpage, xpath='//*[@id="expanded_standings"]/tbody')
tableguy <- html_nodes(webpage, xpath='//*[@id="all_expanded_standings"]')


data.frame(html_table(tableguy, fill=TRUE)[1])
